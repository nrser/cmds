...
describe 'Cmds.tokenize_option' do
  ...
  describe 'false_mode option' do
    context 'false_mode: :negate|:no' do
      context 'false_short_opt_mode: :capitalize|:cap|:upper|:upcase' do
        def f(name, value)
          Cmds.tokenize_option name, value, false_mode: :negate, false_short_opt_mode: :capitalize
        end

        it %(raises ArgumentError if `name` is a single character not [a-z]) do
          expect { f '?', false }.to raise_error ArgumentError
        end
      end # false_short_opt_mode option
    end
  end # false_mode option
end # .tokenize_option
