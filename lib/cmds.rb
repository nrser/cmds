...
class Cmds
  ...
  include Cmds::Log::Mixin

  # Attributes
  # ============================================================================

  # ERB string template (with Cmds-specific extensions) for the command.
  #
  # @return [String]
  #
  ...
  def initialize template, **options
    ...
    options[:kwds][:opts] = options.delete :opts if options.key? :opts
  ...
  def curry *args, **kwds, &input_block
    self.class.new template,
                   ...
                   input: (input_block ? input_block.call : input),
                   assert: assert,
                   env: env,
                   format: self.format,
                   chdir: chdir
  ...
  def render *args, **kwds
    ...
    if env_mode == :inline && !env.empty?
      rendered = env.sort_by { |name, _value|
        name
      }.map { |name, value|
        "#{name}=#{Cmds.esc value}"
      }.join("\n\n") + "\n\n" + rendered
